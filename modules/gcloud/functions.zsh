...
function gcloud-project-django-setup() {
    ...
    if ! gcloud-sql-proxy-and-django-setup --quiet; then
        _log-error "✗ Error starting the Cloud SQL Proxy and applying migrations"
        return 1
    fi

    # Step 7: Build and deploy the service to Cloud Run
    if ! gcloud-run-build-and-deploy-initial --quiet; then
        _log-error "✗ Error building and deploying the service to Cloud Run"
        return 1
    fi

    # Step 8: Setup Cloud Load Balancer
    if ! gcloud-compute-engine-cloud-load-balancer-setup --quiet; then
        _log-error "✗ Error setting up the Cloud Load Balancer"
        return 1
    fi

    # Step 9: Create Cloud Scheduler jobs case exists
    if ! gcloud-scheduler-jobs-sync --quiet; then
        _log-error "✗ Error syncing Cloud Scheduler job"
        return 1
    fi

    _log-success "🎉 Django project in Google Cloud has been successfully set up!"
    # } 2>&1 | tee -a "$log_file"
}
...
function gcloud-project-django-teardown() {
    ...
    gcloud-compute-engine-cloud-load-balancer-teardown --quiet || _log-error "✗ Error deleting the Cloud Load Balancer"

    # Step 3: Delete Cloud Run service
    gcloud-run-service-delete --quiet || _log-error "✗ Error deleting the Cloud Run service"

    # Step 4: Delete Cloud Secret Manager secret
    gcloud-secret-manager-env-delete --quiet || _log-error "✗ Error deleting the Secret Manager secret"

    # Step 5: Delete Cloud Storage buckets
    gcloud-storage-buckets-delete --quiet || _log-error "✗ Error deleting Cloud Storage buckets"

    # Step 6: Delete Artifact Registry repository
    gcloud_artifact_registry_repository_delete --quiet || _log-error "✗ Error deleting Artifact Registry repository"

    # Step 7: Delete Cloud SQL for PostgreSQL instance
    gcloud-sql-instance-delete --quiet || _log-error "✗ Error deleting Cloud SQL instance"
